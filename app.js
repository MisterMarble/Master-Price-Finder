...
function renderCard(row){
  ...
  <input id="shareLink" type="text" readonly class="share-link" />
  <span id="shareToast" class="share-toast" role="status" aria-live="polite"></span>
</div>
`;
const linkField = document.getElementById('shareLink'); if (linkField) linkField.value = buildMaterialLink(row);
wireShareButton(row);
}
...
function wireShareButton(row){
  ...
  if (linkField) linkField.value = buildMaterialLink(row);
if (!btn) return;
    ...
    if (isWebShareAvailable()){
      try{
        await navigator.share({
          title: payload.title,
          text: payload.text, // include link in text for maximum compatibility
          url:  payload.url
        });
        return; // success or user-cancel — nothing more to do
      }catch(e){
        if (e && e.name === 'AbortError') return; // user cancelled
      }
    }

    // Desktop or unsupported -> copy
    try{
      await fallbackCopy(`${payload.title}\n${payload.text}`);
      showToast(toast, 'Link copied.');
    }catch{
      showToast(toast, 'Couldn’t share. Try copying.');
    }
  };
}