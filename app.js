...
function buildSharePayload(row){
  // Prefer the Cloudinary image URL; if none, do NOT fall back to app link
  const link  = (row && row.imageUrl) ? row.imageUrl : "";
  ...
  // If no link, just return text (so email/SMS don’t include your app URL)
  const text = bits || '';
...
const isDesktop = !isWebShareAvailable();
...
btn.onclick = async () => {
  // Mobile / Web Share supported
  if (!isDesktop) {
    try {
      // Share the image URL if present; otherwise share just title/text
  ...
  // Desktop (or mobile fallback): open image URL if present, else tell the user
  if (payload.url) {
    window.open(payload.url, '_blank');
  } else {
    showToast(toast, 'No image link to open.');
  }
};
}